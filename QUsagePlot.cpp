...
#include "QUsagePlot.h"

void QUsagePlot::resizeEvent(QResizeEvent *event)
{
...
QUsagePlot::QUsagePlot(QWidget * parent /*= nullptr*/)
...
void QUsagePlot::setMaximumTime(unsigned int max)
{
    // set time vector
    time.clear();
    for (int i = 0; i <= max; i++)
...
void QUsagePlot::setPlotName(const QString & name)
{
    cornorLabel[TopLeft]->setText(name);
}

void QUsagePlot::setMaximumUsage(double max)
{
    // set axis range
...
void QUsagePlot::setThemeColor(const QColor & themeColor, unsigned int scale)
{
...
void QUsagePlot::setUsageUnit(const QString & unit)
{
    cornorLabel[TopRight]->setText(QString("%1 %2").arg(yAxis2->range().upper).arg(unit));
}

void QUsagePlot::setLabelFont(const QFont & font)
{
    for (QLabel * label : cornorLabel)
        label->setFont(font);
}

void QUsagePlot::setLabelColor(const QColor & color)
{
    for (QLabel * label : cornorLabel)
    {
        label->setStyleSheet(QString("color:rgba(%1,%2,%3,%4);font: 75 10pt \"Arial\";")
            .arg(color.red())
            .arg(color.green())
            .arg(color.black())
            .arg(color.alpha()));
...
void QUsagePlot::addData(double data)
{
    Q_ASSERT_X(data <= yAxis->range().maxRange, "QUsagePlot::addData", "Data overflow");
    Q_ASSERT_X(data >= 0, "QUsagePlot::addData", "Data underflow");
    usage.removeLast();
    usage.prepend(data);
...
QPixmap QUsagePlot::toPixmap(int width, int height, double scale)
{
